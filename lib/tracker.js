...
const rawSuperagent = require('superagent');
...
class Tracker {
  ...
  setup() {
    ...
    this.superagent = options.trace.http ? superagentWrapper.create(this.logger) : rawSuperagent;
    this.router = route(stats.createStats(options.monitor));
    this.monkInspector = new MonkInspector(options.trace.monk ? logger : null, options.app);
