...
const configClient = new ConfigClient(process.env.NODE_ENV);
class Tracker {
  constructor(opts) {
    ...
    this.configClient.setupRemoteConfig(this.options.service, this.options.region);
    this.setup();
  }
  ...
  setup() {
    ...
    this.config = this.configClient.proxy;
  ...
  createKoaV1(koaApp) {
    ...
    koaApp.start = (port, callback) => {
      ...
      if (this.options.config.enable) {
        this.config
          .init()
          .then(() => {
            koaApp.listen(port, () => callback());
          })
          .catch(() => {
            throw new Error('Init config failed!');
          });
      } else {
        koaApp.listen(port, () => callback());
      }
  ...
  createKoaV2(koaApp) {
    ...
    koaApp.start = async (port) => {
      ...
      await new Promise((resolve) => {
        koaApp.server.listen(port, resolve);
      });
    };
...
module.exports.config = configClient.proxy;
