...
function buildLog(requestAt, protocol, traceInfo, host, req) {
  let type = 'http';
...
module.exports.inject = (req, protocol, host, options) => {
  ...
  const logMsg = options.traceLogger ? buildLog(requestAt, protocol, traceInfo, host, req) : null;
  req.on('error', (err) => {
    if (logMsg) {
      logMsg.span.error = err;
      logMsg.span.tags.status_code = err.status;
      logMsg.span.error_stack = err.stack;
      logMsg.process_time = calElapsedTime(startAt);
